...
export function CarDetail({ car }: { car: Car }) {
  ...
  const color: Paint =
    availableColors.find((item) => item.id === colorId) ?? availableColors[0];
    ...
    <article>
                  ...
                  return (
                    <button
                      ...
                      className={`shrink-0 rounded-full border px-4 py-2 text-sm transition-colors ${
                        active
                          ? "border-brand bg-brand text-accent-foreground"
                          : "border-rule bg-mist text-ink hover:border-ink/40"
                      }`}
                    >
                      {item.name[lang]}
                    </button>
              ...
              <div className="mt-3 flex flex-wrap items-start gap-3">
                {availableColors.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={`size-10 rounded-full border-2 shadow-[inset_0_-8px_12px_rgba(0,0,0,0.25)] ${
                          active ? "border-ink" : "border-ink/15 group-hover:border-ink/40"
                        }`}
      ...
      <div className="mx-auto grid max-w-6xl gap-12 px-5 py-14 sm:px-8 lg:grid-cols-[1.15fr_0.85fr]">
        <div>
          ...
          <dl className="mt-8 grid gap-3 sm:grid-cols-2">
            {specs.map((spec) => (
              <div key={spec.label.en} className="rounded-xl border border-rule bg-paper px-5 py-4 text-ink">
                <dt className="text-[11px] font-medium uppercase tracking-[0.16em] text-quiet">
                  {spec.label[lang]}
                </dt>
          ...
          <h2 className="mt-10 font-display text-2xl font-medium">{t.car.highlights}</h2>
        ...
        <aside className="h-fit rounded-xl border border-rule bg-paper p-6 text-ink">
          <h2 className="font-display text-2xl font-medium">{t.car.quoteTitle}</h2>
